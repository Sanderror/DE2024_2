...
df = spark.read \
  .format("bigquery") \
  .load("tranquil-lore-435419-m2.labdataset.retaildata")    # project_id.datatset.tablename. Use your project id
df.printSchema()
df.show(4)
...
bucket = "temp_de2024_2083033"  # use your bucket 
...
ordered_word_count.write.format('bigquery') \
  .option('table', 'tranquil-lore-435419-m2.labdataset.wordcounts') \
  .mode("append") \
  .save()
